...
export class Hashids {
  ...
  public constructor(
    ...
    if (
      [...this.seps].length === 0 ||
      [...this.alphabet].length / [...this.seps].length >
        sepDiv
    ) {
      sepsLength = Math.ceil(
        [...this.alphabet].length / sepDiv,
      );
    ...
    const guardCount = Math.ceil(
      [...this.alphabet].length / guardDiv,
    );
    ...
    if ([...this.alphabet].length < 3) {
      ...
      this.guards = unicodeSubstr(
        this.alphabet,
        0,
        guardCount,
      );
      this.alphabet = unicodeSubstr(
        this.alphabet,
        guardCount,
      );
    }
  ...
  public encode<T extends string | NumberLike>(
    ...
    if (
      !(numbers as NumberLike[]).every(isPositiveAndFinite)
    ) {
      return ret;
    }
  ...
  private _encode(numbers: NumberLike[]): string {
    ...
    if ([...ret].length < this.minLength) {
      const prefixGuardIndex =
        (numbersIdInt + [...ret][0].codePointAt(0)!) %
        guards.length;
      ret = guards[prefixGuardIndex] + ret;

      if ([...ret].length < this.minLength) {
        const suffixGuardIndex =
          (numbersIdInt + [...ret][2].codePointAt(0)!) %
          guards.length;
  ...
  private _decode(id: string): NumberLike[] {
    ...
    const idGuardsArray = splitAtMatch(id, char =>
      this.guards.includes(char),
    );
    const splitIndex =
      idGuardsArray.length === 3 ||
      idGuardsArray.length === 2
        ? 1
        : 0;
    ...
    const idArray = splitAtMatch(rest, char =>
      this.seps.includes(char),
    );
    ...
    const { result } = idArray.reduce(
      ...
      ({ result, lastAlphabet }, subId) => {
        ...
        return {
          result: [
            ...result,
            fromAlphabet(subId, nextAlphabet),
          ],
          lastAlphabet: nextAlphabet,
        };
      },
      {
        result: [] as NumberLike[],
...
export const onlyChars = (
  [...str]: string,
  [...only]: string,
) => str.filter(char => only.includes(char)).join("");

export const unicodeSubstr = (
  [...str]: string,
  from: number,
  to?: number,
) =>
  str
    .slice(from, to === undefined ? undefined : from + to)
    .join("");
...
function shuffle(alphabet: string, [...salt]: string) {
  ...
  for (
    let i = alphabetChars.length - 1, v = 0, p = 0;
    i > 0;
    i--, v++
  ) {
    v %= salt.length;
  ...
  input
    ...
    .reduce((carry, index) => {
      if (typeof carry === "bigint") {
        return (
          carry * BigInt(alphabet.length) + BigInt(index)
        );
      }
      ...
      if (isSafeValue) {
        ...
        if (typeof BigInt === "function") {
          return (
            BigInt(carry) * BigInt(alphabet.length) +
            BigInt(index)
          );
        } else {
...
const safeParseInt10 = (str: string) =>
  safeToParseNumberRegExp.test(str)
    ? parseInt(str, 10)
    : NaN;
