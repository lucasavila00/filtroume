...
function compute_overlap(
  ...
  var areaTotal;
  if (
    whLandscape[0] >= whPortrait[0] &&
    whLandscape[1] >= whPortrait[1]
  ) {
    ...
    areaTotal = whLandscape[0] * whLandscape[1];
    areaTotal +=
      (whPortrait[1] - whLandscape[1]) * whPortrait[0];
  }
...
export const is_portrait = function() {
  try {
    if (
...
export const check_isIOS = function() {
  const isIOS =
...
export const get_IOSVersion = function() {
  const v = navigator["appVersion"].match(
    /OS (\d+)_(\d+)_?(\d+)?/,
  );
  if (!v) {
    return null;
...
export const check_isAndroid = function() {
...
export const get_androidVersion = function() {
  const ua = navigator["userAgent"].toLowerCase();
...
const require_flipVideoWHIfPortrait = function() {
...
const resize_canvasToFullScreen = () => {
  _whCanvasPx = [
    window["innerWidth"],
    window["innerHeight"],
  ];
...
export const size_canvas = function(options: ISizeOptions) {
  ...
  if (!c) {
    setTimeout(() => size_canvas(options), 16);
    return;
  ...
  if (is_portrait() && require_flipVideoWHIfPortrait()) {
  ...
  const dpr = window.devicePixelRatio
    ...
    : 1;
  const whCanvasPxScaled: [number, number] = [
    _whCanvasPx[0] * dpr,
    _whCanvasPx[1] * dpr,
  ];
